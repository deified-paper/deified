...
#define print_helper(app, stats, ret, out, f, ...)                             \
    ret = f(&stats->buffer[stats->size], sizeof(stats->buffer) - stats->size,  \
            __VA_ARGS__);                                                      \
    if (ret < 0) {                                                             \
        pr_warn_ratelimited("Error printing statistics for pid %d!\n",         \
                            app ? app->pid : -1);                              \
        goto out;                                                              \
    } else if (ret >= sizeof(stats->buffer) - stats->size) {                   \
        pr_warn_ratelimited(                                                   \
            "Filled buffer while printing statistics for pid %d!\n",           \
            app ? app->pid : -1);                                              \
...
static int interface_stats_open(struct inode *ip, struct file *fp) {
    ...
    /* Print row headers */
    print_helper(app, stats, ret, out, snprintf, "name,pid");
