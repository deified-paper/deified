...
struct SyscallVisitor : public InstVisitor<SyscallVisitor> {
    ...
    void visitModule(Module &M) {
        ...
        createHQFunctions(IRB, M, nullptr, nullptr, nullptr, nullptr, nullptr,
                          nullptr, nullptr, &SCF, nullptr, nullptr, nullptr);
    }
    ...
    void visitCallBase(CallBase &CB) {
        ...
        if (hasSyscall(*V) && !isHQSyscall(*currentFunc)) {
            // Fetch the syscall number from the system call
            auto *N = getSyscallNum(CB);
            if (!N) {
                errs() << CB;
                report_fatal_error("Unable to fetch system call number!");
            }
            ...
            if (currentFunc->hasFnAttribute(Attribute::AlwaysInline) ||
                ...
                for (auto *U : currentFunc->users()) {
                    if (auto *UCB = dyn_cast<CallBase>(U)) {
                        ...
                        Value *NArg;
                        if (auto *A = dyn_cast<Argument>(N)) {
                            NArg = UCB->getArgOperand(A->getArgNo());
                        } else if (auto *C = dyn_cast<Constant>(N)) {
                            NArg = C;
                        } else {
                            errs() << *UCB << "\n" << CB;
                            report_fatal_error(
                                "Unable to fetch system call number argument!");
                        }
                ...
                if (!createSyscallMessage(IRB, CB, SCF, DT, PDT, N))
