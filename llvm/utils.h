...
// Create a LoadInst from a pointer, which may be of compound type but with only
// one ultimate primitive type.
...
// two-field struct used to track each pointer-value pair. CFI specifies
// whether CFI or DFI.
...
enum GEPBehavior {
    // Stop at GEPs
    NO_GEP,
    // Resolve all-zero GEPs
    ZERO_GEP,
    // Attempt to resolve constant GEPs
    RESOLVE_GEP,
    // Always recurse on GEPs, regardless of arguments
    RECURSE_GEP,
};
// Simplify an instruction, removing casts, GEPs, invariant groups, etc.
const Value *simplify(const Value *V, enum GEPBehavior Behavior = RESOLVE_GEP);
...
// Returns whether the call is a HQ pointer function
bool isHQPointerFunction(const StringRef Name);

// Returns whether the call is an annotated HQ pointer
typedef enum {
    NO_DFI,
    DFI,
    DFI_Private,
...
// Determine if a type is wrapped for DFI instrumentation
bool isWrappedClassTy(const Type &Ty);

// Determine if a type or subtypes are wrapped for DFI instrumentation
bool hasWrappedClassTy(const Type &Ty);

// Determine if a type is a simple pointer type, or a decayed array type
...
// Refines ModRefInfo based on demangling of the symbol name
...
// Determine if a demangled C++ symbol is operator new
bool isOperatorNew(const StringRef Demangled, bool *isArray = nullptr);

// Determine if a demangled C++ symbol is operator delete
bool isOperatorDelete(const StringRef Demangled, bool *isArray = nullptr);
...
// Determine the system call argument from a system call
