...
CallInst *createCastedCall(IRBuilder<> &IRB, FunctionCallee &FC,
    ...
    for (unsigned i = 0; i < FTy->getNumParams(); ++i) {
        assert(Args[i] && "Missing argument in call!");
...
LoadInst *createSingleLoad(IRBuilder<> &IRB, Value *V) {
    Type *I32Ty = IRB.getInt32Ty(), *I64Ty = IRB.getInt64Ty(),
         *Ty = V->getType();
    ...
    if (Ty->isStructTy()) {
        Indexes.push_back(ConstantInt::get(I32Ty, 0));
        while (auto *STy = dyn_cast<StructType>(Ty)) {
            Indexes.emplace_back(ConstantInt::get(I32Ty, 0));
            if (STy->getNumElements() != 1) {
                // Fall-back to casted load
                V = IRB.CreateBitCast(V, PointerType::getUnqual(I64Ty));
                goto out;
            }
        ...
        V = IRB.CreateInBoundsGEP(V, Indexes);
...
void createHQFunctions(IRBuilder<> &IRB, Module &M, FunctionCallee *OCF,
                       FunctionCallee *ODF, FunctionCallee *OIF,
                       FunctionCallee *PCF, FunctionCallee *PCIF,
                       FunctionCallee *PDF, FunctionCallee *PIF,
                       FunctionCallee *SCF, FunctionCallee *PMCF,
                       FunctionCallee *PFF, FunctionCallee *PRF) {
    auto &C = M.getContext();

    // Create basic types
    Type *VTy = IRB.getVoidTy();
    PointerType *PTy = IRB.getInt8PtrTy(), *PPTy = PointerType::getUnqual(PTy);
    IntegerType *I1Ty = IntegerType::getInt1Ty(C),
                *I64Ty = IntegerType::getInt64Ty(C);
    ...
    INTERFACE_TYPE == INTERFACE_TYPE_ZERO
        ...
        if (PDF)
            *PDF = M.getOrInsertFunction(STR(POINTER_DEFINE_FUNCTION), ALP, VTy,
                                         PPTy, PTy, I1Ty);
        if (PIF) {
            auto ALI =
                AttributeList::get(C, {{AttributeList::FirstArgIndex, RNNC},
        ...
        if (SCF) {
            ...
            *SCF =
                M.getOrInsertFunction(STR(SYSCALL_FUNCTION), ALS, VTy, I64Ty);
        }
        if (PMCF)
    ...
    if (PFF || PRF) {
        std::pair<unsigned, Attribute> NC = {
            AttributeList::FirstArgIndex,
            Attribute::get(C, Attribute::NoCapture)};

        if (PFF) {
            auto ALFF = AttributeList::get(C, {NC});
    ...
    if (OCF || ODF || OIF) {
        Attribute NCRO[] = {Attribute::get(C, Attribute::NoCapture),
                            Attribute::get(C, Attribute::ReadOnly)};
        auto ASNCR = AttributeSet::get(C, NCRO);
        auto ALNCRO =
            AttributeList::get(C, AttributeList::FirstArgIndex, ASNCR);

        if (OCF)
            *OCF = M.getOrInsertFunction(STR(OBJECT_CHECK_FUNCTION), ALNCRO,
                                         VTy, PTy, I64Ty);
        if (ODF)
            *ODF = M.getOrInsertFunction(STR(OBJECT_DEFINE_FUNCTION), ALNCRO,
                                         VTy, PTy, I64Ty, I1Ty);
        if (OIF)
            *OIF = M.getOrInsertFunction(STR(OBJECT_INVALIDATE_FUNCTION),
                                         ALNCRO, VTy, PTy, I64Ty);
    }
}
...
bool createSyscallMessage(IRBuilder<> &IRB, CallBase &CB, FunctionCallee &SCF,
    ...
    BasicBlock *BB = CB.getParent(),
               *NBB = isa<Instruction>(Num)
                          ? cast<Instruction>(Num)->getParent()
                          : nullptr;
    ...
    // before CB in its basic block. This must also be located after Num, if it
    // is an instruction.
    if (DT && PDT && NBB != BB) {
        ...
        for (auto it = idf_begin(BB), end = idf_end(BB); it != end; ++it) {
            ...
            if (!Earliest) {
                assert((!NBB || DT->getNode(NBB)) && DT->getNode(*it) &&
                       DT->getNode(BB) && PDT->getNode(BB) &&
                       PDT->getNode(*it) &&
                       "Basic blocks must be in dominator trees!");
                // Find the earliest satisfying basic block
                if ((!NBB || DT->dominates(NBB, *it)) &&
                    DT->dominates(*it, BB) && PDT->dominates(BB, *it))
                    Earliest = *it;
            }
    ...
    if (!InsertPt) {
        ...
        while (prevI && prevI != Num && !isa<AllocaInst>(prevI) &&
               (!isa<CallBase>(prevI) || isa<IntrinsicInst>(prevI)) &&
               !isa<PHINode>(prevI)) {
            InsertPt = prevI;
    ...
    IRB.CreateCall(SCF, {Num});
    return true;
}

void createGlobalInit(Module &M, bool CFI,
    ...
    FunctionCallee InitFunction = M.getOrInsertFunction(
        CFI ? STR(CFI_INIT_GLOBALS_FUNCTION) : STR(DFI_INIT_GLOBALS_FUNCTION),
        ALI, VTy, PI64Ty, I64Ty, PI64Ty, I64Ty);
...
void instrumentGlobalVariables(
    Module &M, StructType &STy, bool CFI, bool LTO,
    const ArrayRef<const Value *> Exclude,
    ...
    for (GlobalVariable &GV : M.globals()) {
        ...
        auto FPs = unpackValue(*GV.getInitializer(), CFI,
                               CFI ? !isVTable(Demangled) : false, Exclude);
        for (auto &P : FPs) {
            // No values found
            if (!P.first)
                continue;
...
const Value *simplify(const Value *V, enum GEPBehavior Behavior) {
    while (true) {
        if (auto *BCO = dyn_cast<BitCastOperator>(V)) {
            ...
            if (Behavior == NO_GEP)
                return V;
            else if (Behavior == RECURSE_GEP) {
                V = GEP->getPointerOperand();
                continue;
            }

            auto *NewV = resolveGEP(GEP, Behavior == RESOLVE_GEP);
            if (NewV) {
                V = NewV;
                continue;
            }
...
bool isHQPointerFunction(const StringRef Name) {
    return Name.startswith("__hq_pointer_");
}

HQAnnotatedType isHQAnnotatedPointer(const IntrinsicInst *II) {
    if (II->getIntrinsicID() == Intrinsic::ptr_annotation) {
        const auto Str =
            cast<ConstantDataSequential>(
                cast<GlobalVariable>(cast<GEPOperator>(II->getArgOperand(1))
                                         ->getPointerOperand())
                    ->getInitializer())
                ->getAsCString();
        if (Str.equals("hq_private"))
            return DFI_Private;
...
Type *getNonDecayedType(const Value *V) {
    if (auto *GEP = dyn_cast<GEPOperator>(V)) {
        // Use original array type if pointer is decayed
        auto *GEPVTy = GEP->getPointerOperand()->getType();
        if (GEPVTy->getPointerElementType()->isArrayTy())
            return GEPVTy->getPointerElementType();
    }

    return V->getType()->getPointerElementType();
}

bool isWrappedClassTy(const Type &Ty) {
    if (!Ty.isStructTy())
        return false;
    const auto &Name = Ty.getStructName();
    return Name.contains("hq_wrapper") || Name.contains("hq_basic_string");
}

bool hasWrappedClassTy(const Type &Ty) {
    if (!Ty.isPointerTy())
        return false;
    const auto *ATy = Ty.getPointerElementType();
    if (!ATy->isAggregateType())
        return false;

    DedupVector<const Type *, 2> Stack;
    Stack.push_back(ATy);

    while (!Stack.empty()) {
        auto *STy = Stack.pop_back_val();
        if (isWrappedClassTy(*STy))
            return true;
        for (const auto *Ty : STy->subtypes()) {
...
ModRefInfo refineBehaviorDemangled(ModRefInfo MRI, const StringRef Demangled,
                                   unsigned ArgNo, bool isConstruct,
                                   bool isDestruct) {
    const bool isOpAssign = isOperatorAssign(Demangled);

    if (ArgNo == 0) {
        if (isOpAssign) {
            // Read/write may occur on object assigned to
        } else if (isConstruct) {
            // Only write may occur on object constructed
            MRI = clearRef(MRI);
        } else if (isDestruct) {
            // Read/write may occur on object destructed
        } else if (Demangled.endswith("const")) {
            // Only read may occur on 'const' member function
            MRI = clearMod(MRI);
        }
    } else {
        ssize_t Count = 0;
        ...
        for (size_t i = Demangled.size() - 1; i < Demangled.size(); --i) {
            if (Demangled[i] == ')') {
                if (!Count)
                    End = i;
                ++Count;
        ...
        // Shift by one for the 'this' argument
        ArgNo -= 1;
        // Split into individual arguments
        SmallVector<StringRef, 2> Args;
        Demangled.substr(Begin, End - Begin + 1).split(Args, ',');
        if (ArgNo >= Args.size())
            return MRI;

        // FIXME: Matching for 'const' is brittle
        if (isOpAssign) {
            // Read/write may occur on object assigned from, unless const
            if (Args[ArgNo].find("const") <
                std::min(Args[ArgNo].find('*'), Args[ArgNo].find('&')))
                MRI = clearMod(MRI);
        } else if (isConstruct) {
            // Read/write may occur on object constructed from, unless const
            if (Args[ArgNo].find("const") <
                std::min(Args[ArgNo].find('*'), Args[ArgNo].find('&')))
                MRI = clearMod(MRI);
        } else if (isDestruct) {
            report_fatal_error("Destructor should only have one argument!");
        }
    }
...
bool isTypeInfo(const StringRef Demangled) {
    return Demangled.rfind("typeinfo for ") == 0;
}

bool isTypeInfoName(const StringRef Demangled) {
    return Demangled.rfind("typeinfo name for ") == 0;
}

bool isVirtualThunk(const StringRef Demangled) {
    return Demangled.rfind("virtual thunk to ") == 0;
}

bool isVTable(const StringRef Demangled) {
    return Demangled.find("vtable for ") != std::string::npos;
}

bool isVTableTable(const StringRef Demangled) {
    return Demangled.rfind("VTT for ") == 0;
}

bool isOperatorAssign(const StringRef Demangled) {
    return Demangled.rfind("::operator=") != std::string::npos;
}

bool isOperatorNew(const StringRef Demangled, bool *isArray) {
    if (isArray)
        *isArray = (Demangled.rfind("operator new[]") == 0);
    return Demangled.rfind("operator new") == 0;
}

bool isOperatorDelete(const StringRef Demangled, bool *isArray) {
    if (isArray)
...
/* System calls */
// FIXME: Assembly containing conditional control-flow
...
Value *getSyscallNum(const CallBase &CB) {
    const std::string whitelist[] = {
        // Plain system call
        "syscall",
        // chromium/src/third_party/tcmalloc/chromium/src/base/linux_syscall_support.h
        "movq $5,%r10;syscall",
        "movq $5,%r10;movq $6,%r8;syscall",
        "movq $5,%r10;movq $6,%r8;movq $7,%r9;syscall",
        // llvm-project/compiler-rt/lib/sanitizer_common/sanitizer_linux.cpp
        "mov $5, %r10;syscall",
        "mov $5, %r10;mov $6, %r8;syscall",
        "mov $5, %r10;mov $6, %r8;mov $7, %r9;syscall",
    };
    auto *IA = dyn_cast<InlineAsm>(CB.getCalledOperand());
    const auto AsmString = StringRef(IA->getAsmString()).rtrim();
    // FIXME: Improve parsing of inline assembly
    if (!IA || none_of(whitelist, [&AsmString](const std::string &entry) {
            return AsmString == entry;
        })) {
        // chromium/src/sandbox/linux/seccomp-bpf/syscall.cc
        auto *F = CB.getFunction();
        if (F->hasFnAttribute(Attribute::Naked) && F->getName() == "SyscallAsm")
            return F->getArg(0);
        return nullptr;
    }
    unsigned ArgNo = 0;
    auto CIV = IA->ParseConstraints();
    // See TargetLowering.cpp for argument handling
    for (const auto &CI : CIV) {
        if (CI.Type == InlineAsm::isOutput && CI.isIndirect)
            ++ArgNo;
        else if (CI.Type == InlineAsm::isInput) {
            if (CI.Codes.size() == 1 &&
                (CI.Codes[0] == "{ax}" || CI.Codes[0] == "0"))
                return ArgNo < CB.arg_size() ? CB.getArgOperand(ArgNo)
                                             : nullptr;
            ++ArgNo;
        }
    }
