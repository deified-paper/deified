...
static const Value *getRootVariable(const Value *Pointer) {
    ...
    Pointer = simplify(Pointer, ZERO_GEP);
                ...
                if (CB->isArgOperand(&U)) {
                    ...
                    if (CB->hasFnAttr(Attribute::InaccessibleMemOnly) ||
                        ...
                        if (IF == F) {
                            ...
                            if (CorrectOffset && CF &&
                                isHQPointerFunction(CF->getName()))
...
struct FinalizeVisitor : public InstVisitor<FinalizeVisitor> {
    ...
    void visitModule(Module &M) {
        ...
        createHQFunctions(IRB, M, nullptr, nullptr, nullptr, &PCF, &PCIF, &PDF,
                          &PIF, nullptr, &PMCF, &PFF, &PRF);
        ...
        if (!CF)
            ...
            if (auto *GEP = dyn_cast<GEPOperator>(Root)) {
                ...
                Parent = const_cast<Value *>(
                    simplify(Root->stripInBoundsConstantOffsets(), ZERO_GEP));
            }
    ...
    void visitIntrinsicInst(IntrinsicInst &II) {
        ...
        case Intrinsic::memmove: {
            Value *A0 = const_cast<Value *>(
                      simplify(II.getArgOperand(0), ZERO_GEP)),
                  *A1 = const_cast<Value *>(
                      simplify(II.getArgOperand(1), ZERO_GEP)),
...
static bool finalizeCFIInstrumentation(Module &M, CallGraph *CG,
    ...
    if (RunCFI) {
        ...
        const Function *DelVirt = SkipCXXVirtualError
                                      ...
                                      : nullptr;
        instrumentGlobalVariables(M, *STy, true, LTOTime, {PureVirt, DelVirt},
    ...
    if (FV.visit(M) && hasGlobals && RunCFI) {
        createGlobalInit(M, true, InitGlobalsInternal, InitGlobalsExternal);
