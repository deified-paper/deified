...
void OBJECT_CHECK_FUNCTION(const void *ptr, uint64_t sz) {
#ifndef NDEBUG
    printf("Checking object %p with size 0x%lx...\n", ptr, sz);
#endif /* NDEBUG */

    if (!interface.send_msgn(HQ_MSG_CHECK_BLOCK, ptr, sz)) {
        constexpr static char err[] = "Error sending OBJECT_CHECK_BLOCK!\n";
...
void OBJECT_DEFINE_FUNCTION(const void *ptr, uint64_t sz, bool priv) {
#ifndef NDEBUG
    printf("Defining object %p with size 0x%lx...\n", ptr, sz);
#endif /* NDEBUG */

    if (!interface.send_msgn(priv ? HQ_MSG_DEFINE_BLOCK_PRIVATE
                                  : HQ_MSG_DEFINE_BLOCK,
                             ptr, sz)) {
        constexpr static char err[] = "Error sending OBJECT_DEFINE_BLOCK!\n";
        RAW_SYSCALL(3, SYS_write, STDERR_FILENO,
                    reinterpret_cast<uintptr_t>(err), sizeof(err));
        RAW_SYSCALL(1, SYS_exit_group, -1);
    }
...
void OBJECT_REALLOC_FUNCTION(const void *dst, const void *src, uint64_t sz) {
#ifndef NDEBUG
    printf("Moving object %p to %p with size 0x%lx...\n", src, dst, sz);
#endif /* NDEBUG */
    ...
    if (!interface.send_msg2(
        ...
        constexpr static char err[] = "Error sending OBJECT_BLOCK_MOVE!\n";
        RAW_SYSCALL(3, SYS_write, STDERR_FILENO,
                    reinterpret_cast<uintptr_t>(err), sizeof(err));
        RAW_SYSCALL(1, SYS_exit_group, -1);
    }
...
void OBJECT_INVALIDATE_FUNCTION(const void *ptr, uint64_t sz) {
#ifndef NDEBUG
    printf("Invalidating object %p with size 0x%lx...\n", ptr, sz);
#endif /* NDEBUG */

    if (!interface.send_msg2(HQ_MSG_INVALIDATE_BLOCK,
                             reinterpret_cast<uintptr_t>(ptr), sz)) {
        constexpr static char err[] =
            "Error sending OBJECT_BLOCK_INVALIDATE!\n";
        RAW_SYSCALL(3, SYS_write, STDERR_FILENO,
                    reinterpret_cast<uintptr_t>(err), sizeof(err));
        RAW_SYSCALL(1, SYS_exit_group, -1);
    }
...
void POINTER_CHECK_FUNCTION(const void **pp, const void *p) {
    ...
    if (!interface.send_msg2(HQ_MSG_CHECK,
...
void POINTER_CHECK_INVALIDATE_FUNCTION(const void **pp, const void *p) {
    ...
    if (!interface.send_msg2(HQ_MSG_CHECK_INVALIDATE,
                             reinterpret_cast<const uintptr_t>(pp),
                             reinterpret_cast<const uintptr_t>(p))) {
        constexpr static char err[] =
...
void POINTER_DEFINE_FUNCTION(const void **pp, const void *p, bool priv) {
#ifndef NDEBUG
    printf("Defining pointer %p with value %p...\n", pp, p);
#endif /* NDEBUG */

    if (!interface.send_msg2(priv ? HQ_MSG_DEFINE_PRIVATE : HQ_MSG_DEFINE,
                             reinterpret_cast<const uintptr_t>(pp),
                             reinterpret_cast<const uintptr_t>(p))) {
...
void SYSCALL_FUNCTION(uint64_t num) {
    if (!interface.send_msg1(HQ_MSG_SYSCALL, gettid())) {
...
init_globals_function(const hq_init_t *init_int, const size_t int_sz,
    ...
    if (int_sz >= HQ_GLOBALS_INTERNAL_THRESHOLD) {
        ...
        if (base && !interface.send_msg1(HQ_MSG_INIT_GLOBALS,
                                         base - sizeof(Elf_Ehdr))) {
            constexpr static char err[] = "Error sending INIT_GLOBALS!\n";
            RAW_SYSCALL(3, SYS_write, STDERR_FILENO,
                        reinterpret_cast<uintptr_t>(err), sizeof(err));
            RAW_SYSCALL(1, SYS_exit_group, -1);
        }
        ...
        for (size_t i = 0; i < int_sz; ++i)
            POINTER_DEFINE_FUNCTION(
                reinterpret_cast<const void **>(init_int[i].ptr),
                reinterpret_cast<const void *>(init_int[i].val), false);
    }
    ...
    for (size_t i = 0; i < ext_sz; ++i) {
        POINTER_DEFINE_FUNCTION(
            ...
            reinterpret_cast<const void *>(init_ext[i].val), false);
    }
}

void INIT_GLOBALS_FUNCTION(const hq_init_t *init_int, const size_t int_sz,
                           const hq_init_t *init_ext, const size_t ext_sz) {
